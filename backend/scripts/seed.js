...
async function seed() {
    try {
        ...
        const users = [
            { first: 'Param', last: 'User', email: 'param@example.com' },
        ...
        for (const t of transactionData) {
            // Find the default wallet payment method for this customer
            const pMethod = await methodRepo.findOne({ where: { customer_id: t.customer.id, method_type: 'wallet' } });
            
            await txnRepo.save(txnRepo.create({
                txn_id: uuidv4(),
                customer_id: t.customer.id,
                merchant_id: nexStore.id,
                receiver_id: nexStore.id,
                receiver_type: 'merchant',
                payment_method_id: pMethod ? pMethod.id : 1,
                amount: t.amount,
                currency: 'INR',
                status: 'success',
                mode: 'wallet',
