...
/**
 * List all settlements for Admin review
 */
exports.listSettlements = async (req, res, next) => {
    try {
        const { status } = req.query;
        const settleRepo = AppDataSource.getRepository(entities.settlements);
        
        const qb = settleRepo.createQueryBuilder("s")
            .leftJoinAndSelect("s.merchants", "merchant")
            .leftJoinAndSelect("s.bank_accounts", "bank_accounts")
...
exports.updateSettlementStatus = async (req, res, next) => {
    try {
        ...
        if (!['pending', 'processing', 'completed', 'failed'].includes(status)) {
            return res.status(400).json(error('VALIDATION', 'Invalid settlement status'));
        }

        const settleRepo = AppDataSource.getRepository(entities.settlements);
        const settlement = await settleRepo.findOneBy({ id: parseInt(id) });
        
        if (!settlement) {
            return res.status(404).json(error('NOT_FOUND', 'Settlement not found'));
        }

        const previousStatus = settlement.status;
        await settleRepo.update(id, { 
            status, 
            settlement_date: status === 'completed' ? new Date() : settlement.settlement_date 
        });

        // 🚀 NEW: Simulate Real-Time Bank Transfer
        if (status === 'completed' && previousStatus !== 'completed' && settlement.bank_account_id) {
            const bankRepo = AppDataSource.getRepository(entities.bank_accounts);
            const bankAccount = await bankRepo.findOneBy({ id: settlement.bank_account_id });
            
            if (bankAccount) {
                const currentBalance = parseFloat(bankAccount.balance || 0);
                const payoutAmount = parseFloat(settlement.net_amount || 0);
                
                await bankRepo.update(bankAccount.id, {
                    balance: currentBalance + payoutAmount,
                    updated_at: new Date()
                });
                
                console.log(`🏦 [PAYOUT_REALTIME] Credited ₹${payoutAmount} to account ending in ${bankAccount.account_last_four}`);
            }
        }

        res.json(success('Settlement status updated and funds transferred to bank account'));
    } catch (err) {
        next(err);
    }
};
