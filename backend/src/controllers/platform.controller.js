...
const { transactions, customers, credit_cards, bank_accounts, payment_methods, merchant_apps, api_keys, payment_sessions, api_logs, verification_codes } = require('../entities/entities');
const { v4: uuidv4 } = require('uuid');
const crypto = require('crypto');
const { success, error } = require('../utils/responseHelper');
const emailService = require('../services/email.service');

// ── Database-backed Gateway OTP Logic ──────────────────────────────

const generatePaymentOtp = () => String(Math.floor(100000 + Math.random() * 900000));

const savePaymentOtp = async (email, otp) => {
...
const verifyPaymentOtpDB = async (email, inputCode) => {
    ...
    if (!entry) return { valid: false, reason: 'Incorrect OTP. Please check and try again.' };
...
exports.createPaymentSession = async (req, res, next) => {
    try {
        ...
        if (merchantApp.app_name.includes('NexStore')) {
...
exports.processPayment = async (req, res, next) => {
    try {
        ...
        if (req.user) {
            ...
            // ── GUEST PAYMENT OTP GATE ─────────────────────────────────────
            // All guest (non-logged-in) payments require email OTP verification.
            if (!otp_code) {
                // First call — no OTP yet: generate, save to DB, and email it
                const otp = generatePaymentOtp();
                await savePaymentOtp(email, otp);
                await emailService.sendOTP(email, otp, 'payment');
                console.log(`📧 [GATEWAY OTP] Sent to ${email}: ${otp}`);
                return res.json(success('OTP sent to email', { requires_otp: true }));
            }

            // Second call — OTP provided: verify before proceeding
            const { valid, reason } = await verifyPaymentOtpDB(email, otp_code);
            if (!valid) {
                return res.status(400).json(error('INVALID_OTP', reason));
            }
            // ── OTP verified — continue with payment ──────────────────────
...
exports.getPayouts = async (req, res, next) => {
    try {
        ...
        const payouts = await settlementRepo.find({
            where: { merchant_id: merchantId },
            order: { created_at: 'DESC' }
        });
        console.log(`📜 [PAYOUTS_FETCH] MerchantID: ${merchantId}, Found: ${payouts.length}`);

        res.json(success('Payouts retrieved successfully', payouts));
    } catch (err) {
        next(err);
    }
};
...
exports.getPayoutBalance = async (req, res, next) => {
    try {
        const merchantId = req.user.id;
        const balance = await calculateMerchantBalance(merchantId);
        console.log(`💰 [BALANCE_FETCH] MerchantID: ${merchantId}, Balance: ${balance}`);
        res.json(success('Balance retrieved', { available_balance: balance }));
    } catch (err) {
        next(err);
    }
};
...
exports.createPayout = async (req, res, next) => {
    try {
        ...
        if (!amount || parseFloat(amount) <= 0) {
            return res.status(400).json(error('VALIDATION', 'Amount must be greater than zero'));
        }

        const availableBalance = await calculateMerchantBalance(merchantId);
        if (parseFloat(amount) > availableBalance) {
            return res.status(400).json(error('INSUFFICIENT_FUNDS', `Maximum available for payout is ₹${availableBalance.toLocaleString()}`));
        }

        const settlementRepo = AppDataSource.getRepository('settlements');
        const settlement = await settlementRepo.save({
            settlement_id: `SET-${uuidv4().substring(0, 8).toUpperCase()}`,
            merchant_id: merchantId,
            bank_account_id: bank_account_id ? parseInt(bank_account_id) : null,
            total_amount: parseFloat(amount),
            fee_amount: 0, // Zero fee for MVP
        ...
        res.status(201).json(success('Payout request submitted', settlement));
    } catch (err) {
        next(err);
    }
};
...
/**
 * Internal helper to calculate current withdrawable balance
 */
async function calculateMerchantBalance(merchantId) {
    const txnRepo = AppDataSource.getRepository('transactions');
    const { settlements } = require('../entities/entities'); // Use explicit entity
    const settleRepo = AppDataSource.getRepository(settlements);

    // 1. Total Revenue from successful payments (Gateway + Direct Transfers)
    const revenueRes = await txnRepo.createQueryBuilder("txn")
        .where("(txn.merchant_id = :merchantId OR (txn.receiver_id = :merchantId AND txn.receiver_type = 'merchant')) AND txn.status = :status", { merchantId, status: 'success' })
        .select("SUM(txn.amount)", "total")
        .getRawOne();
    
    const totalRevenue = parseFloat(revenueRes?.total || revenueRes?.TOTAL || 0);

    // 2. Total Paid Out / Pending Settlement
    const settledRes = await settleRepo.createQueryBuilder("s")
        .where("s.merchant_id = :merchantId AND s.status != :status", { merchantId, status: 'failed' })
        .select("SUM(s.total_amount)", "total")
        .getRawOne();
    
    const totalSettled = parseFloat(settledRes?.total || settledRes?.TOTAL || 0);

    return Math.max(0, totalRevenue - totalSettled);
}
