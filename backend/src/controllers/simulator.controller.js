    ...
    try {
        ...
        if (result.status === 'success') {
            ...
            await notificationService.createNotification(
                ...
                req.user.type, 
                'Payment Successful', 
                `Successfully paid ₹${parseFloat(result.amount).toLocaleString()} using ${result.payment_method.type}`,
                'payment'
            );
...
exports.getReceivers = async (req, res, next) => {
    try {
        ...
        const { merchants, customers } = require('../entities/entities');
        const customerId = req.user.id;
...
exports.processGuestPayment = async (req, res, next) => {
    try {
        ...
        const { customers, credit_cards, bank_accounts, payment_methods } = require('../entities/entities');
        ...
        if (result.status === 'success') {
            // Trigger notification for guest (as customer)
            await notificationService.createNotification(
                customerId,
                'customer',
                'Payment Sent',
                `You successfully paid ₹${parseFloat(result.amount).toLocaleString()} via payment link.`,
                'payment'
            );
