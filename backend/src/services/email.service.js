...
class EmailService {
    ...
    /**
     * Sends a transaction receipt for successful payments.
     */
    async sendReceipt(email, txn) {
        const subject = `Payment Receipt - ${txn.txn_id.substring(0, 8)}`;
        
        const htmlContent = `
            <div style="font-family: sans-serif; max-width: 550px; margin: auto; padding: 25px; border: 1px solid #f1f5f9; border-radius: 16px; background-color: #ffffff; color: #1e293b;">
                <div style="text-align: center; margin-bottom: 30px;">
                    <h1 style="color: #6366f1; margin: 0; font-size: 28px; font-weight: 800; letter-spacing: -1px;">PaySim</h1>
                    <p style="color: #64748b; margin: 5px 0 0; font-size: 14px; font-weight: 500;">Transaction Successful</p>
                </div>
                
                <div style="background: #f8fafc; padding: 20px; border-radius: 12px; margin-bottom: 25px;">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 12px;">
                        <span style="color: #64748b; font-size: 13px; font-weight: 600; text-transform: uppercase;">Transaction ID</span>
                        <span style="color: #334155; font-size: 13px; font-family: monospace; font-weight: 700;">${txn.txn_id}</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; margin-bottom: 12px; align-items: center;">
                        <span style="color: #64748b; font-size: 13px; font-weight: 600; text-transform: uppercase;">Amount Paid</span>
                        <span style="color: #0f172a; font-size: 18px; font-weight: 800;">₹${parseFloat(txn.amount).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                    </div>
                </div>

                <div style="margin-bottom: 25px;">
                    <h3 style="color: #0f172a; font-size: 14px; font-weight: 700; margin-bottom: 12px; border-bottom: 1px solid #f1f5f9; padding-bottom: 8px;">Order Details</h3>
                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                        <span style="color: #64748b; font-size: 13px;">Merchant</span>
                        <span style="color: #1e293b; font-size: 13px; font-weight: 600;">${txn.merchant_name || 'NexStore Online'}</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                        <span style="color: #64748b; font-size: 13px;">Date</span>
                        <span style="color: #1e293b; font-size: 13px; font-weight: 600;">${new Date().toLocaleString()}</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                        <span style="color: #64748b; font-size: 13px;">Payment Status</span>
                        <span style="color: #166534; font-size: 13px; font-weight: 700;">SUCCESS</span>
                    </div>
                </div>

                <div style="text-align: center; margin-top: 30px; font-size: 12px; color: #94a3b8; border-top: 1px solid #f1f5f9; pt: 20px;">
                    <p>Thank you for shopping with <strong>${txn.merchant_name || 'NexStore'}</strong>!</p>
                    <p style="margin-top: 5px;">&copy; 2026 PaySim Inc. All rights reserved.</p>
                </div>
            </div>
        `;
