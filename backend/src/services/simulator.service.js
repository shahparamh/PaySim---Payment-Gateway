...
const { customers, verification_codes, merchants, wallets, transactions, payment_methods, credit_cards, bank_accounts } = require('../entities/entities');
...
async function processPayment({ customerId, paymentMethodId, amount, pin, receiverId, receiverType, otpCode }) {
    try {
        // --- 0. High Value and Forced OTP Check ---
        const threshold = parseFloat(process.env.MIN_HIGH_VALUE_TRANSACTION) || 100000;
        let isForcedOtp = false;

        if (receiverType === 'merchant') {
            const merchantRepo = AppDataSource.getRepository(merchants);
            const merchant = await merchantRepo.findOne({ where: { id: parseInt(receiverId) } });
            if (merchant && (merchant.name === 'NexStore' || merchant.name === 'NexStore Online')) {
                isForcedOtp = true;
            }
        }

        if (amount >= threshold || isForcedOtp) {
            if (!otpCode) {
        ...
        try {
            ...
            try {
                ...
                if (user && receiverType === 'merchant') {
                    const merchantRepo = AppDataSource.getRepository(merchants);
                    const merchant = await merchantRepo.findOne({ where: { id: parseInt(receiverId) } });
                    
                    if (merchant && merchant.business_name.includes('NexStore')) {
                        console.log(`[Receipt] Sending email receipt to ${user.email} for NexStore Simulator transaction ${resultTxnId}`);
                        const emailService = require('./email.service');
                ...
                console.error('⚠️ [Receipt] Failed to send email receipt in simulator:', receiptErr.message);
            }
        } catch (err) {
