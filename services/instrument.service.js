...
async function removeBankAccount(accountId, customerId) {
    const bankRepo = AppDataSource.getRepository(bank_accounts);
    await bankRepo.update({ id: accountId, customer_id: customerId }, { status: 'closed' });

    const pmRepo = AppDataSource.getRepository(payment_methods);
    await pmRepo.update({ instrument_id: accountId, method_type: 'bank_account', customer_id: customerId }, { status: 'disabled' });
}
